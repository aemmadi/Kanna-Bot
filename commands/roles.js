...
module.exports.run = async (bot, message, args) =>{
  ...
  if(!rName)
    return message.channel.send(`Couldn't find the role ${role} on this server!`);
  ...
  try {
    await rMember.send(`Congrats, you have been given the role **'${rName.name}'** on the server **'${message.guild.name}'**`); //Informs added user of his new role (DM)
  }catch(e){
    message.channel.send(`Congrats, <@${rMember.id}>, you have been given the role ${rName.name}. We tried to DM, but ran into an error.`); //Send message in server if DM fails
    }
  }
  ...
  //Checks if its addRole or removeRole
    ...
    try {
      await rMember.send(`You have been removed from the role **'${rName.name}'** on the server **'${message.guild.name}'**`); //Informs user of his removal from role (DM)
    }catch(e){
      message.channel.send(`You have been removed from the role '${rName.name}' on the server '${message.guild.name}'. We tried to DM, but ran into an error`); //Sends message in server if DM fails
    }
  }
  else{
    return message.channel.send("Make sure you got the syntax right.\n`!role [add/rmv] <usermention> {role-name}`"); //Error message
  }
}
